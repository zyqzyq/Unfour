...
1. **Lint warning cleanup**
   ...
   - Prerequisites: None.
   - Acceptance criteria: `pnpm run lint` produces fewer warnings than the current baseline (65); no new errors introduced; component rendering unchanged.
   - Independent commit: Yes.
   - Evidence: `pnpm run lint` output — 65 warnings confirmed at commit `9c99e0f`. Primary sources: `ApiDebuggerPage.tsx` (react-hooks/refs), `WorkspaceDialogs.tsx` (set-state-in-effect), `useLayoutPersistence.ts` (exhaustive-deps), `utils.tsx` (only-export-components). See PROJECT_STATE.md Lint warnings, OPEN_ISSUES.md P2.
...
2. **Semantic token replacement in App.tsx**
   ...
   - Independent commit: Yes.
   - Evidence: `apps/desktop/src/App.tsx` at commit `9c99e0f` — hardcoded `slate-*`, `white`, `rose-*`, `teal-*` classes present. See PROJECT_STATE.md Partially Implemented (UI module split).
   - Recommended model: weaker cheaper model is sufficient
...
3. **Workspace dialog extraction from App.tsx**
   ...
   - Independent commit: Yes.
   - Evidence: `apps/desktop/src/App.tsx` at commit `9c99e0f` — workspace dialogs and window controls inline. See PROJECT_STATE.md Partially Implemented (UI module split).
...
4. **PostgreSQL live driver phase 2 — live verification**
   ...
   - Evidence: OPEN_ISSUES.md P2 (PostgreSQL live verification) — `NOT VERIFIED` at commit `9c99e0f`. PROJECT_STATE.md Known Limitations confirms no live PostgreSQL available in scan environment.
