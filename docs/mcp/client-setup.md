...
3. Replace `PASTE_ABSOLUTE_PATH_FROM_SETTINGS_MCP` in one configuration below
   ...
   or JSON.
4. Start the desktop app before connecting the MCP client, then restart the
   client after saving its configuration.
...
details, and the client must retry only after the target and payload have been
reviewed.
