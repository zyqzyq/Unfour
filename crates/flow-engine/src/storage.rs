...
impl FlowService {
    ...
    pub(crate) async fn insert_run(&self, run: &FlowRun) -> AppResult<()> {
        sqlx::query("INSERT INTO flow_runs (id, workspace_id, flow_id, status, run_json, started_at, updated_at, finished_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").bind(&run.id).bind(&run.workspace_id).bind(&run.flow_id).bind(run.status.as_str()).bind(safe_run(run, &self.persistence.values())?).bind(&run.started_at).bind(&run.started_at).bind(&run.finished_at).execute(self.db.pool()).await?;
        Ok(())
    }
    pub(crate) async fn persist_run(&self, run: &FlowRun) -> AppResult<()> {
        let result = sqlx::query("UPDATE flow_runs SET status = ?, run_json = ?, finished_at = ?, updated_at = ? WHERE workspace_id = ? AND id = ? AND status = 'running'").bind(run.status.as_str()).bind(safe_run(run, &self.persistence.values())?).bind(&run.finished_at).bind(chrono::Utc::now().to_rfc3339()).bind(&run.workspace_id).bind(&run.id).execute(self.db.pool()).await?;
...
fn safe_run(run: &FlowRun, persistence_secrets: &[String]) -> AppResult<String> {
    ...
    let mut secrets = persistence_secrets.to_vec();
