CREATE TABLE pro_sync_outbox_protocol_v4 (
  account_id TEXT NOT NULL,
  local_workspace_id TEXT NOT NULL,
  cloud_workspace_id TEXT NOT NULL,
  entity_type TEXT NOT NULL CHECK (entity_type IN (
    'workspace', 'connection', 'workspaceVariable', 'workspaceEnvironment',
  ...
  CHECK (
    ...
    OR (
      entity_type IN (
        'connection', 'apiCollection', 'apiFolder', 'apiRequest',
        'sshTask', 'sshTaskStep'
      )
...
INSERT INTO pro_sync_outbox_protocol_v4 (
...
CREATE TABLE pro_sync_entity_state_protocol_v4 (
  account_id TEXT NOT NULL,
  cloud_workspace_id TEXT NOT NULL,
  entity_type TEXT NOT NULL CHECK (entity_type IN (
    'workspace', 'connection', 'workspaceVariable', 'workspaceEnvironment',
...
INSERT INTO pro_sync_entity_state_protocol_v4 (
...
ALTER TABLE pro_sync_outbox_protocol_v4 RENAME TO pro_sync_outbox;
ALTER TABLE pro_sync_entity_state_protocol_v4 RENAME TO pro_sync_entity_state;
...
ADD COLUMN connection_v4_bootstrap_state TEXT NOT NULL DEFAULT 'pending'
CHECK (connection_v4_bootstrap_state IN ('pending', 'completed'));
