...
import { inheritedPairs, pairs, queryOccurrences, type Pair } from "./actionAuthoring";
...
export function ApiActionEditor(props: ActionEditorProps) {
  ...
  return <div className="grid gap-2">
    ...
    {(["headers", "query"] as const).map((key) => {
      ...
      const rawRows = pairs(action.arguments[argument] ?? []);
      const rows = key === "query" && !legacy && rawRows ? queryOccurrences(rawRows) : rawRows;
      ...
      return <fieldset key={key} className="grid gap-1"><legend>{t(`flow.argument.${key}`)}</legend>
        ...
        <div className="text-xs text-[var(--u-color-text-muted)]">{inherited.map((pair, i) => <div key={i} className="flex items-center justify-between gap-1"><span className="min-w-0 break-all">{pair.key}{key === "query" ? ` [${inherited.slice(0, i).filter((previous) => previous.key === pair.key).length + 1}]` : ""}: {String(pair.value)} · {t("flow.inherited")}{pair.enabled === false ? ` · ${t("flow.disabled")}` : ""}</span>{!legacy && rows && !rows.some((row) => key === "headers" ? row.key.toLowerCase() === pair.key.toLowerCase() : row.key === pair.key && row.occurrence === inherited.slice(0, i).filter((previous) => previous.key === pair.key).length) && <Button size="sm" variant="ghost" onClick={() => set(argument, [...rows, { ...pair, ...(key === "query" ? { occurrence: inherited.slice(0, i).filter((previous) => previous.key === pair.key).length } : {}) }])}>{t("flow.override")}</Button>}</div>)}</div>
        {rows ? <>{rows.map((pair, index) => {
          ...
          return <div key={index} className="grid grid-cols-[1fr_2fr_auto] items-start gap-1">
            <div className="flex items-center gap-1">{!legacy && key === "query" && pair.occurrence !== undefined && <span className="shrink-0 text-xs text-[var(--u-color-text-muted)]">[{pair.occurrence + 1}]</span>}<Input aria-label={`${t(`flow.argument.${key}`)} ${index + 1} · ${t("flow.fieldName")}`} value={pair.key} onChange={(e) => update({ key: e.target.value, ...(key === "query" && !legacy ? { occurrence: undefined } : {}) })} /></div>
