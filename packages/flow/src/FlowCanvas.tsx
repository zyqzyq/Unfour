...
export function FlowCanvas({ definition, onChange, disabled = false, readOnly = false, selected, onSelect, onRemove, run }: {
  ...
  return <section className="flow-canvas flex min-h-0 min-w-0 flex-1 flex-col border-b border-[var(--u-color-border)]" aria-label={t("flow.canvas.title")}>
    <div className="flex flex-wrap items-center gap-2 p-2">
      {!readOnly && <><Button variant="ghost" size="sm" disabled={disabled || !selectedStep || definition.steps.length <= 1} onClick={() => {
    ...
    <p className="px-2 pb-2 text-xs text-[var(--u-color-text-muted)]">{t(readOnly ? "flow.runCanvasHelp" : "flow.canvas.help")}</p>
